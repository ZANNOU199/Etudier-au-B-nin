...
export const CMSProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const apiRequest = async (endpoint: string, options: RequestInit = {}, forceNoToken: boolean = false) => {
    const isPublic = endpoint === '/majors' || endpoint === '/universities';
    const useAuth = token && !forceNoToken;
    ...
    if (token) {
      try {
        ...
        setApplications(rawApps.map((a: any) => {
          ...
          const uName = a.major?.university?.acronym || a.institution?.acronym || a.university?.acronym || a.university_name || a.universityName || "Université non renseignée";
          const mId = (a.major_id || a.filiere_id || a.majorId)?.toString();
        ...
        console.warn("Échec récupération candidatures:", e.message);
      }
    }
