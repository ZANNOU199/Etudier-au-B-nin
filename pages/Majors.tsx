...
const Majors: React.FC = () => {
  ...
  const cities = useMemo(() => {
    const allCities = majors.map(m => m.location).filter(Boolean);
  ...
  const filtered = useMemo(() => {
    ...
    return majors.filter(m => {
      ...
      const facName = (m.facultyName || '').toLowerCase();
      const uniName = (m.universityName || '').toLowerCase();
        ...
        <div className="max-w-5xl mx-auto flex flex-col items-center gap-8">
          ...
          <div className="w-full max-w-4xl bg-white/5 backdrop-blur-xl border border-white/10 p-2 rounded-3xl shadow-2xl flex flex-col md:flex-row gap-2">
             <div className="flex-1 flex items-center h-16 px-6 bg-white/5 rounded-2xl">
                ...
                <input 
                  className="bg-transparent border-none text-white focus:ring-0 flex-1 font-bold px-4 outline-none" 
      ...
      <div className="container mx-auto px-4 py-16 max-w-7xl">
          <div className="flex flex-col md:flex-row justify-between items-center mb-10 pb-6 border-b border-gray-100 dark:border-gray-800">
             <h2 className="text-2xl font-black dark:text-white tracking-tight">
               {isLoading ? 'Chargement...' : `${filtered.length} Formations disponibles`}
             </h2>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              ...
              {filtered.length === 0 && !isLoading && (
                 <div className="col-span-full text-center py-20 bg-white dark:bg-surface-dark rounded-[40px] border-2 border-dashed border-gray-100 dark:border-gray-800">
                    <span className="material-symbols-outlined text-6xl text-gray-200 mb-4">search_off</span>
                    <h3 className="text-xl font-black dark:text-white">Aucune formation trouvée</h3>
