...
const ApplyProcess: React.FC = () => {
  const { user, addApplication } = useCMS();
  ...
  const handleFinalSubmit = () => {
    // Création de l'application réelle
    addApplication({
      id: 'EAB-24-' + Math.floor(Math.random() * 10000),
      ...
      progress: 65,
      documents: ['BAC_Releve.pdf', 'ID_Card.png']
    });
    setStep(4);
  };
    ...
    <div className="min-h-screen bg-[#f8faf9] dark:bg-background-dark flex flex-col font-display">
      <nav className="bg-white dark:bg-surface-dark border-b border-gray-100 dark:border-gray-800 px-6 py-4 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-4">
           ...
           <div>
             <h2 className="font-black dark:text-white tracking-tighter text-sm uppercase">Nouveau Dossier</h2>
          ...
          <div className="bg-white dark:bg-surface-dark rounded-[48px] shadow-2xl overflow-hidden border border-gray-100 dark:border-gray-800">
            ...
            {step === 2 && (
              <div className="p-10 md:p-16 space-y-10 animate-in fade-in slide-in-from-right-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-3">
                    <label className="text-xs font-black uppercase tracking-widest text-gray-400">INE / Matricule</label>
            ...
            {step === 3 && (
              <div className="p-10 md:p-16 space-y-10 animate-in fade-in slide-in-from-right-4 text-center">
                <h2 className="text-3xl font-black dark:text-white">Téléversement</h2>
                <p className="text-gray-500">Uploadez vos documents (PDF ou JPG)</p>
                <div className="py-12 border-4 border-dashed border-gray-100 dark:border-gray-800 rounded-[32px] flex flex-col items-center gap-4 text-gray-300 hover:text-primary hover:border-primary transition-all cursor-pointer">
                   <span className="material-symbols-outlined text-6xl">cloud_upload</span>
                   <p className="font-bold uppercase text-[10px] tracking-widest">Relevé de notes BAC & Identité</p>
                </div>
                <div className="flex justify-between pt-8">
                   <button onClick={() => setStep(2)} className="text-xs font-black text-gray-400 uppercase tracking-widest">Retour</button>
                   <button onClick={handleFinalSubmit} className="bg-primary px-10 py-4 rounded-2xl font-black text-black uppercase tracking-widest text-xs">Terminer</button>
                </div>
              </div>
            )}
            ...
            {step === 4 && (
              <div className="p-10 md:p-20 flex flex-col items-center text-center gap-8 animate-in zoom-in-95">
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-gray-500 max-w-md mx-auto font-medium text-lg leading-relaxed">Votre dossier est désormais entre les mains de l'administration de <span className="text-primary">{selectedMajor.universityName}</span>.</p>
                </div>
                <button onClick={() => navigate('/dashboard')} className="px-16 py-5 bg-primary text-black rounded-2xl font-black uppercase tracking-widest text-xs shadow-2xl hover:scale-105 transition-all">
