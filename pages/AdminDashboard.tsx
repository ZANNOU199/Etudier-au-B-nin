...
type CMSSection = 'content' | 'theme' | 'languages' | 'media' | 'roles';

const AdminDashboard: React.FC = () => {
  const { 
    content, languages, themes, translate, updateContent, 
    toggleLanguage, applyTheme, updateTheme, activeTheme, userRole 
  } = useCMS();
  
  const [activeView, setActiveView] = useState<AdminView>('overview');
  const [activeCMSSection, setActiveCMSSection] = useState<CMSSection>('content');
              ...
              <div className="hidden sm:block space-y-0.5">
                 <h1 className="text-xl lg:text-2xl font-black dark:text-white tracking-tighter uppercase">
                    {activeView === 'overview' ? 'Administration' : 
                     activeView === 'cms' ? 'Interface CMS' : activeView}
                 </h1>
        ...
        <div className="p-4 lg:p-12 space-y-10">
          ...
          {activeView === 'cms' && (
            <div className="space-y-10 animate-fade-in">
               {/* CMS Sub-navigation */}
               <div className="flex flex-wrap gap-4 bg-white dark:bg-surface-dark p-2 rounded-2xl shadow-sm border border-gray-100 dark:border-white/5">
                  {[
                    ...
                    { id: 'media', label: 'Médiathèque', icon: 'photo_library' }
                  ].filter(s => !s.restricted).map(section => (
                    <button 
                      key={section.id}
                      onClick={() => setActiveCMSSection(section.id as CMSSection)}
                      className={`flex items-center gap-3 px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${activeCMSSection === section.id ? 'bg-primary text-black' : 'text-gray-400 hover:bg-gray-50 dark:hover:bg-white/5'}`}
                    >
               ...
               {activeCMSSection === 'content' && (
                 <div className="grid grid-cols-1 gap-6">
                    {Object.keys(content).map(key => (
                      <div key={key} className="bg-white dark:bg-surface-dark p-8 rounded-3xl border border-gray-100 dark:border-white/5 shadow-sm space-y-6">
                        <div className="flex justify-between items-center border-b border-gray-50 dark:border-white/5 pb-4">
                           <span className="text-[10px] font-black text-primary uppercase tracking-widest">Variable: {key}</span>
                           <span className="material-symbols-outlined text-gray-300">key</span>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                           {languages.filter(l => l.isActive).map(lang => (
                             <div key={lang.code} className="space-y-2">
                               <label className="text-[9px] font-black text-gray-400 uppercase tracking-[0.2em]">{lang.label}</label>
                               <textarea 
                                 value={content[key][lang.code] || ''}
                                 onChange={(e) => updateContent(key, lang.code, e.target.value)}
                                 className="w-full p-4 rounded-xl bg-gray-50 dark:bg-white/5 border border-transparent focus:border-primary outline-none font-medium dark:text-white resize-none"
                                 rows={2}
                               />
                             </div>
                           ))}
                    ...
                    <div className="flex justify-end">
                       <button className="bg-primary text-black font-black px-12 py-4 rounded-2xl shadow-xl shadow-primary/20 hover:scale-105 transition-all text-[11px] uppercase tracking-widest">
               ...
               {activeCMSSection === 'theme' && (
                 <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                       {themes.map(theme => (
                         <div key={theme.id} className={`bg-white dark:bg-surface-dark p-8 rounded-3xl border-2 transition-all ${theme.isActive ? 'border-primary shadow-xl' : 'border-transparent opacity-60'}`}>
                            <div className="flex justify-between items-start mb-6">
                            ...
                            <div className="flex gap-3 mb-8">
                               <div className="size-10 rounded-lg shadow-inner border border-white/10" style={{ backgroundColor: theme.primary }}></div>
                               <div className="size-10 rounded-lg shadow-inner border border-white/10" style={{ backgroundColor: theme.background }}></div>
                               <div className="size-10 rounded-lg shadow-inner border border-white/10" style={{ backgroundColor: theme.surface }}></div>
                            </div>
                            <button 
                              onClick={() => applyTheme(theme.id)}
                              className={`w-full py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${theme.isActive ? 'bg-primary text-black' : 'bg-gray-50 dark:bg-white/5 text-gray-400 hover:text-primary'}`}
                            >
                    ...
                    {/* Theme Customizer */}
                    <div className="bg-white dark:bg-surface-dark p-8 rounded-3xl border border-gray-100 dark:border-white/5 shadow-premium space-y-8">
                       <h3 className="text-sm font-black dark:text-white uppercase tracking-widest border-b border-gray-50 dark:border-white/5 pb-4">Personnaliser {activeTheme.name}</h3>
                       <div className="space-y-6">
                          ...
                          <div className="space-y-3">
                             <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Rayon de bordure (Radius)</label>
                             <input 
                                type="text"
               ...
               {activeCMSSection === 'languages' && (
                 <div className="bg-white dark:bg-surface-dark p-10 rounded-[40px] border border-gray-100 dark:border-white/5 shadow-sm space-y-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                       {languages.map(lang => (
                         <div key={lang.code} className="flex items-center justify-between p-6 bg-gray-50 dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/5">
                            <div className="flex items-center gap-4">
                               <div className="size-12 bg-white dark:bg-surface-dark rounded-xl flex items-center justify-center font-black text-primary shadow-sm uppercase">{lang.code}</div>
                               <div>
                                  <p className="font-black dark:text-white">{lang.label}</p>
                                  <p className="text-[10px] text-gray-400 uppercase tracking-widest">{lang.isActive ? 'Activée' : 'Désactivée'}</p>
                               </div>
                            </div>
                            <button 
                              onClick={() => toggleLanguage(lang.code)}
                              className={`size-12 rounded-xl flex items-center justify-center transition-all ${lang.isActive ? 'bg-primary text-black' : 'bg-gray-200 dark:bg-gray-800 text-gray-500'}`}
                            >
                    ...
                    <div className="pt-6">
                       <button className="flex items-center gap-3 px-8 py-4 bg-gray-100 dark:bg-white/5 border border-dashed border-gray-300 dark:border-gray-700 rounded-2xl font-black text-[10px] uppercase tracking-widest text-gray-400 hover:text-primary hover:border-primary transition-all">
