                  ...
                  <div className="bg-[#0d1b13] p-10 rounded-[48px] border border-white/5 space-y-10 shadow-2xl">
                    <div className="overflow-x-auto rounded-[32px] border border-white/5 bg-white/2">
                       <table className="w-full text-left border-collapse min-w-[1000px]">
                          ...
                          <tbody className="divide-y divide-white/5">{pagedMajors.map(major => (<tr key={major.id} className="hover:bg-white/2 transition-colors"><td className="px-8 py-5 text-left"><div className="space-y-1"><p className="text-white font-black text-sm">{major.name}</p><p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest"><span className="text-amber-400">{major.level}</span> • {major.duration}</p></div></td><td className="px-8 py-5 text-left"><div className="space-y-1"><p className="text-sm font-black text-gray-300 leading-none">{major.universityName}</p><p className="text-[9px] text-gray-500 font-bold italic truncate max-w-[200px]">{major.facultyName}</p></div></td><td className="px-8 py-5 text-left"><span className="px-3 py-1 rounded-lg bg-white/5 border border-white/5 text-[9px] font-black text-primary uppercase tracking-widest">{major.domain}</span></td><td className="px-8 py-5 text-right"><div className="flex justify-end gap-3"><button onClick={() => openWizardForEditMajor(major)} className="size-9 rounded-xl bg-white/5 text-gray-500 hover:text-primary transition-all flex items-center justify-center border border-white/5" title="Modifier"><span className="material-symbols-outlined text-lg">edit</span></button><button onClick={() => { if(window.confirm('Supprimer ?')) deleteMajor(major.id); }} className="size-9 rounded-xl bg-white/5 text-gray-500 hover:text-red-500 transition-all flex items-center justify-center border border-white/5" title="Supprimer"><span className="material-symbols-outlined text-lg">delete</span></button></div></td></tr>))}</tbody>
                       </table>
                    </div>
