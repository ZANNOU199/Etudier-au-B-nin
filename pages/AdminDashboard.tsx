...
// Pagination Constants for Catalog
const UNI_ITEMS_PER_PAGE = 4;
const MAJOR_ITEMS_PER_PAGE = 3;
...
const AdminDashboard: React.FC = () => {
  ...
  // Logic: Catalog Filtering & Pagination
  ...
  const totalUniPages = Math.ceil(filteredUnis.length / UNI_ITEMS_PER_PAGE);
  const pagedUnis = filteredUnis.slice((uniPage - 1) * UNI_ITEMS_PER_PAGE, uniPage * UNI_ITEMS_PER_PAGE);

  const totalMajorPages = Math.ceil(majors.length / MAJOR_ITEMS_PER_PAGE);
  const pagedMajors = majors.slice((majorPage - 1) * MAJOR_ITEMS_PER_PAGE, majorPage * MAJOR_ITEMS_PER_PAGE);
                  ...
                  {applications.map(app => (
                    <div key={app.id} className="bg-white dark:bg-surface-dark p-6 rounded-[32px] border border-gray-100 dark:border-white/5 flex flex-col md:flex-row items-center justify-between gap-6 group hover:shadow-lg transition-all">
                       ...
                       <div className="flex items-center gap-4 w-full md:w-auto">
                          ...
                          <button onClick={() => setSelectedApp(app)} className="size-11 rounded-xl bg-gray-100 dark:bg-white/5 text-gray-400 hover:text-primary flex items-center justify-center transition-all shrink-0">
                              ...
                              <div className="flex justify-between items-start mb-6">
                                 <div className="flex items-center gap-4">
                                    ...
                                    <div className="space-y-1">
                                       ...
                                       <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{major.universityName}</p>
                                    </div>
                                 </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                     <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-4">
                           ...
                           <button className="size-12 rounded-xl bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 transition-colors hover:bg-red-500 group"><div className="size-6 rounded-full bg-white/20 mx-auto group-hover:bg-white transition-colors"></div></button>
                        </div>
                     </div>
                     <div className="space-y-4">
          ...
          <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
             <div className="bg-white dark:bg-surface-dark w-full max-w-2xl rounded-[48px] shadow-2xl overflow-hidden my-auto animate-in zoom-in-95 duration-300">
                <div className="bg-gray-50 dark:bg-white/5 px-10 py-8 flex items-center justify-between border-b border-gray-100 dark:border-white/10">
                   <div>
                      <h3 className="text-2xl font-black dark:text-white tracking-tight leading-none">Nouvel Établissement</h3>
                ...
                <div className="p-8 md:p-12 space-y-10">
                     ...
                     <div className="space-y-8 animate-in slide-in-from-right-4">
                        ...
                        <form onSubmit={(e) => {
                           ...
                           <div className="md:col-span-2 space-y-2">
                              ...
                              <input name="name" required className="w-full p-4 rounded-2xl bg-gray-50 dark:bg-white/5 border-none font-bold dark:text-white outline-none focus:ring-2 focus:ring-primary/20" placeholder="Ex: Université d'Abomey-Calavi" />
                           </div>
                           <div className="space-y-2">
                              <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Sigle</label>
                              <input name="acronym" required className="w-full p-4 rounded-2xl bg-gray-50 dark:bg-white/5 border-none font-bold dark:text-white outline-none focus:ring-2 focus:ring-primary/20" placeholder="Ex: UAC" />
                           </div>
                           <div className="space-y-2">
                              <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Ville</label>
                              <input name="location" required className="w-full p-4 rounded-2xl bg-gray-50 dark:bg-white/5 border-none font-bold dark:text-white outline-none focus:ring-2 focus:ring-primary/20" placeholder="Ex: Cotonou" />
                           </div>
                   ...
                   {wizardStep === 'majors' && (
                     <div className="space-y-8 animate-in slide-in-from-right-4">
                        <div className="text-center space-y-2">
                           <h4 className="text-2xl font-black dark:text-white tracking-tight leading-none">Offre Académique</h4>
                           <p className="text-gray-400 font-medium text-sm">Ajoutez les filières avec leurs débouchés et pré-requis.</p>
                        </div>
                        <form onSubmit={(e) => {
                           e.preventDefault();
                           const fd = new FormData(e.currentTarget);
                           ...
                           <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                              <input name="career" required placeholder="Débouché (obligatoire)" className="w-full p-4 rounded-xl bg-white dark:bg-surface-dark border-none font-bold text-sm dark:text-white" />
                              <input name="diploma" required placeholder="Diplôme requis (obligatoire)" className="w-full p-4 rounded-xl bg-white dark:bg-surface-dark border-none font-bold text-sm dark:text-white" />
                           </div>
                           <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                              <select name="level" className="p-4 rounded-xl bg-white dark:bg-surface-dark border-none font-bold text-xs dark:text-white">
                                 <option value="Licence">Licence</option>
                                 <option value="Master">Master</option>
                                 <option value="Doctorat">Doctorat</option>
                              </select>
                              <input name="fees" required placeholder="Frais (ex: 450k)" className="p-4 rounded-xl bg-white dark:bg-surface-dark border-none font-bold text-xs dark:text-white" />
                              <input name="duration" required placeholder="Durée" className="p-4 rounded-xl bg-white dark:bg-surface-dark border-none font-bold text-xs dark:text-white" />
                           </div>
                        ...
                        <div className="max-h-32 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                           {currentInstMajors.map(m => (
                              <div key={m.id} className="p-3 bg-white dark:bg-white/5 rounded-2xl flex justify-between items-center border border-gray-50 dark:border-white/10 animate-fade-in">
                                 <p className="font-black text-xs dark:text-white">{m.name}</p>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 pt-6 border-t border-gray-100 dark:border-white/10">
                           ...
                           <button onClick={() => setShowWizard(false)} className="flex-1 py-4 bg-black dark:bg-white text-white dark:text-black font-black uppercase text-[10px] tracking-widest rounded-2xl shadow-xl transition-all">Terminer la création</button>
                        </div>
                     </div>
                   )}
                </div>
