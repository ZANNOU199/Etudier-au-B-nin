...
const AdminDashboard: React.FC = () => {
  ...
  // LOGIQUE CATALOGUE UNIQUEMENT (Périmètre autorisé)
    ...
    <div className="flex h-screen w-full bg-[#f4f7f6] dark:bg-background-dark font-display overflow-hidden relative">
      ...
      <main className="flex-1 overflow-y-auto h-screen bg-gray-50 dark:bg-background-dark/50 flex flex-col">
        ...
        <div className="p-4 lg:p-12 space-y-10">
                  ...
                  {applications.map(app => (
                    <div key={app.id} className="bg-white dark:bg-surface-dark p-6 rounded-[32px] border border-gray-100 dark:border-white/5 flex flex-col md:flex-row items-center justify-between gap-6 group hover:shadow-lg transition-all">
                       ...
                       <div className="flex items-center gap-4 w-full md:w-auto">
                          ...
                          <button onClick={() => setSelectedApp(app)} className="size-11 rounded-xl bg-gray-100 dark:bg-white/5 text-gray-400 hover:text-primary flex items-center justify-center transition-all">
          ...
          {/* VIEW: CATALOG (REFACTORED - ONLY ALLOWED PERIMETER) */}
          {activeView === 'catalog' && (
            <div className="space-y-8 animate-fade-in">
               ...
               {/* CATALOG CONTAINER - DARK THEME SPECIFIC (#0d1b13) */}
               {activeCatalogSection === 'universities' && (
                  <div className="bg-[#0d1b13] p-10 rounded-[48px] border border-white/5 space-y-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {pagedUnis.map(uni => (
                        <div key={uni.id} className="bg-white/5 p-6 rounded-[32px] border border-white/5 flex flex-col md:flex-row items-center justify-between gap-6 group hover:bg-white/10 transition-all h-full">
                           <div className="flex items-center gap-6 flex-1 w-full">
                              <div className="size-20 rounded-2xl bg-white/10 flex items-center justify-center p-3 border border-white/10 relative">
                      ...
                      <button onClick={() => { setShowWizard(true); setWizardStep('institution'); setCurrentInstId(null); }} className="min-h-[140px] flex items-center justify-center gap-6 rounded-[32px] border-2 border-dashed border-primary/20 hover:bg-primary/5 transition-all group">
                        ...
                        <span className="font-black uppercase text-[10px] tracking-[0.2em] text-primary">Nouveau établissement</span>
                      </button>
                    </div>
          ...
          <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
             <div className="bg-[#162a1f] w-full max-w-2xl rounded-[48px] shadow-2xl overflow-hidden my-auto animate-in zoom-in-95 duration-300 border border-white/5">
                ...
                <div className="p-8 md:p-12 space-y-10">
                     ...
                     <div className="space-y-8 animate-in slide-in-from-right-4">
                        ...
                        <form onSubmit={(e) => {
                           ...
                           const data: University = {
                              ...
                              description: 'Établissement académique supérieur.',
                           ...
                           <div className="md:col-span-2 space-y-2">
                              ...
                              <input name="name" required className="w-full p-4 rounded-2xl bg-white/5 border-none font-bold text-white outline-none focus:ring-2 focus:ring-primary/20" placeholder="Ex: Université de Parakou" />
                           </div>
                           <div className="space-y-2">
                              <label className="text-[10px] font-black uppercase text-gray-500 tracking-widest">Sigle</label>
                              <input name="acronym" required className="w-full p-4 rounded-2xl bg-white/5 border-none font-bold text-white outline-none focus:ring-2 focus:ring-primary/20" placeholder="Ex: UP" />
                           </div>
                           <div className="space-y-2">
                              <label className="text-[10px] font-black uppercase text-gray-500 tracking-widest">Ville</label>
                              <input name="location" required className="w-full p-4 rounded-2xl bg-white/5 border-none font-bold text-white outline-none focus:ring-2 focus:ring-primary/20" placeholder="Ex: Parakou" />
                           </div>
                           <div className="md:col-span-2 pt-6">
                              <button type="submit" className="w-full py-5 bg-primary text-black font-black rounded-2xl text-[11px] uppercase tracking-widest shadow-xl shadow-primary/20 transition-all">Continuer vers l'étape 2</button>
                           </div>
                        </form>
                     </div>
                   )}
                   {wizardStep === 'faculties' && (
                     <div className="space-y-8 animate-in slide-in-from-right-4 text-white">
                        <div className="text-center space-y-2">
                           <h4 className="text-2xl font-black text-white tracking-tight">Composantes rattachées</h4>
                           <p className="text-gray-400 font-medium text-sm">Ajoutez les écoles ou facultés internes de l'université.</p>
                        </div>
                        <form onSubmit={(e) => {
                           e.preventDefault();
                           const fd = new FormData(e.currentTarget);
                           ...
                           <input name="fName" required placeholder="Nom de la composante (ex: EPAC, ENEAM...)" className="w-full p-4 rounded-xl bg-white/5 border-none font-bold text-white" />
                           <button type="submit" className="w-full py-3 border border-primary/20 text-primary font-black rounded-xl text-[10px] uppercase tracking-widest hover:bg-primary hover:text-black transition-all">+ Ajouter une autre entité</button>
                        </form>
                   ...
                   {wizardStep === 'majors' && (
                     <div className="space-y-8 animate-in slide-in-from-right-4 text-white">
                        <div className="text-center space-y-2"><h4 className="text-2xl font-black text-white tracking-tight">Offre Académique</h4><p className="text-gray-400 font-medium text-sm">Ajoutez les filières avec leurs débouchés et diplômes requis.</p></div>
                        <form onSubmit={(e) => {
                           e.preventDefault();
                           const fd = new FormData(e.currentTarget);
                           const majorData: Major = {
                              id: 'maj-' + Date.now(), name: fd.get('mName') as string, universityId: currentInstId || '', universityName: currentUni?.name || '', facultyName: fd.get('fName') as string, domain: fd.get('domain') as string || 'Général', level: fd.get('level') as any || 'Licence', duration: fd.get('duration') as string || '3 Ans', fees: fd.get('fees') as string || '450.000 FCFA', location: currentUni?.location || '', image: 'https://images.unsplash.com/photo-1517694712202-14dd9538aa97?auto=format&fit=crop&q=80&w=400', careerProspects: [{ title: fd.get('career') as string, icon: 'work' }], requiredDiplomas: [{ name: fd.get('diploma') as string, icon: 'school' }]
                           };
                           addMajor(majorData); e.currentTarget.reset();
                        }} className="space-y-4 p-6 bg-white/5 rounded-[32px] border border-white/5">
                           <input name="mName" required placeholder="Nom de la filière" className="w-full p-4 rounded-xl bg-white/5 border-none font-bold text-white" />
                           ...
                           <button type="submit" className="w-full py-3 bg-white/10 text-primary border border-primary/20 font-black rounded-xl text-[10px] uppercase tracking-widest hover:bg-primary hover:text-black transition-all">+ Enregistrer cette filière</button>
                        </form>
                        <div className="max-h-32 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 pt-6 border-t border-white/5">
                           ...
                           <button onClick={() => setShowWizard(false)} className="flex-1 py-4 bg-primary text-black font-black uppercase text-[10px] tracking-widest rounded-2xl shadow-xl transition-all">Terminer la création</button>
                        </div>
                     </div>
                   )}
                </div>
        ...
        {/* MODAL: APPLICATION DOSSIER (INTACT) */}
        {selectedApp && (
             ...
             <div className="bg-white dark:bg-surface-dark w-full max-w-2xl rounded-[48px] overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
                <div className="p-10 space-y-8">
                   ...
                   <div className="flex flex-wrap gap-3 pt-6 border-t border-gray-100 dark:border-white/10">
                      <button onClick={() => { updateApplicationStatus(selectedApp.id, 'Validé'); setSelectedApp(null); }} className="flex-1 min-w-[140px] py-4 bg-primary text-black font-black rounded-2xl text-[10px] uppercase tracking-widest shadow-xl shadow-primary/20 transition-all hover:scale-105">Valider</button>
                      <button onClick={() => { updateApplicationStatus(selectedApp.id, 'Rejeté'); setSelectedApp(null); }} className="flex-1 min-w-[140px] py-4 bg-red-500 text-white font-black rounded-2xl text-[10px] uppercase tracking-widest shadow-xl transition-all hover:scale-105">Rejeter</button>
                      <button onClick={() => setSelectedApp(null)} className="flex-1 min-w-[140px] py-4 bg-gray-100 dark:bg-white/10 text-gray-500 dark:text-gray-300 font-black rounded-2xl text-[10px] uppercase tracking-widest transition-all">Fermer</button>
                   </div>
                </div>
             </div>
          </div>
