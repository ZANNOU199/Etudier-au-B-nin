...
import { University, Major, CareerProspect, RequiredDiploma, Faculty } from '../types';
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (lines.length < 2) throw new Error("Fichier CSV vide ou mal formé.");
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          // 1. Gérer l'Établissement (Université ou École)
          let uni = tempUnis.find(u => u.acronym.toLowerCase() === row.sigle_inst.toLowerCase());
          
          if (!uni) {
            const newId = 'uni-' + Math.random().toString(36).substr(2, 9);
            uni = {
              id: newId,
              ...
              description: "Établissement importé via CSV.",
          ...
          // 2. Gérer la Faculté / Ecole interne
          ...
          // 3. Créer la Filière
          const prospects: CareerProspect[] = row.debouche 
            ? row.debouche.split('|').map((d: string) => ({ title: d.trim(), icon: 'work' }))
            : [];
          
          const diplomas: RequiredDiploma[] = row.diplome 
            ? row.diplome.split('|').map((d: string) => ({ name: d.trim(), icon: 'school' }))
            : [{ name: 'BAC', icon: 'school' }];
